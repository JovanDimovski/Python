...
def helper(cities):
    dt = datetime.now()
    real_time=(dt.minute*60+dt.second)*1000000+dt.microsecond
    ...
    ############################################################################
        dt = datetime.now()
        pdt=(dt.minute*60+dt.second)*1000000+dt.microsecond
        
        table = []
        table = process_data.process_data_all_st(city)
        
        dt = datetime.now()
        pdt=((dt.minute*60+dt.second)*1000000+dt.microsecond-pdt)/1000
        ...
        print("Main time: ", ct," miliseconds")
        print("Process_data time: ", pdt,"miliseconds")
        main_time+=ct
        ############################################
        ...
        qlen+=3
        ##
        t = threading.Thread(target=draw_env, args = (q,ihi,1,-25,55,"hi",city[0],city[2],3))
        t.daemon = True
        t.start()
        ##
        t = threading.Thread(target=draw_env, args = (q,ilo,1,-40,40,"lo",city[0],city[2],3))
        t.daemon = True
        t.start()
        ##
        min_val, max_val = minmax.min_max2(ipr,"pr")
        if min_val != max_val:
            t = threading.Thread(target=draw_env, args = (q,ipr,1,min_val,max_val,"pr",city[0],city[2],3))
            t.daemon = True
            t.start()
            qlen+=1
        ##
        t = threading.Thread(target=draw_env, args = (q,icc,1,0,8,"cc",city[0],city[2],4))
        t.daemon = True
        t.start()
        ##
        #print(qlen)
    ...
    ############################################
    dt = datetime.now()
    real_time=float((dt.minute*60+dt.second)*1000000+dt.microsecond-real_time)/1000000
...
cities.append("Zakynthos")
cities.append("Thessaloniki")
